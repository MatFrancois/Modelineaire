...
Les gènes H19, IGF2, NNAT, BLCAP qui encodent des enzymes impliquées dans la régulation de la ??????????????????la régul de quoi du coup??????????????????????????????????? et le développement du corps et du cerveau font partie de la famille des gènes soumis à empreinte, c’est à dire que la copie héritée de la mère et la copie héritée du père ne sont pas exprimées de la même manière.
...
- La méthode d'imputation des valeurs manquantes chez l'expression des gènes est restée la même à une différence près : pour les gènes NDK2, NDK3, NDK4 nous avons imputé par une régression linéaire entre eux (NDK2~NDK4; NDK3~NDK2; NDK4~NDK2), en effet ces variables sont très coréllées, ça nous a donc semblé sensé d'utiliser cette méthode.
